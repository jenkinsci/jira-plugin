...
public class JiraEnvironmentVariableBuilderTest {
  ...
  public void setIssueSelectorPersists() {
    final JiraEnvironmentVariableBuilder builder = new JiraEnvironmentVariableBuilder(
        issueSelector);
  ...
  public void performWithNoSiteFailsBuild() throws InterruptedException, IOException {
    JiraEnvironmentVariableBuilder builder = spy(
        new JiraEnvironmentVariableBuilder(issueSelector));
  ...
  public void performAddsAction() throws InterruptedException, IOException {
    ...
    JiraEnvironmentContributingAction action = (JiraEnvironmentContributingAction) (captor
        .getValue());

    assertThat(action.getJiraUrl(), is(JIRA_URL));
    assertThat(action.getIssuesList(),
        anyOf(is(EXPECTED_JIRA_ISSUES_1), is(EXPECTED_JIRA_ISSUES_2)));
  }
