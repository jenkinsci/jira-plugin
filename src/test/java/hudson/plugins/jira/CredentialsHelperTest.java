...
public class CredentialsHelperTest {
  ...
  public void lookupSystemCredentials()
    ...
    StandardUsernamePasswordCredentials c =
        new UsernamePasswordCredentialsImpl(CredentialsScope.SYSTEM, null, null, "username",
            "password");
    ...
    assertEquals(c,
        CredentialsHelper.lookupSystemCredentials(c.getId(), new URL("http://example.org")));
  }
  ...
  public void lookupSystemCredentialsWithDomainRestriction()
    ...
    assertEquals(c,
        CredentialsHelper.lookupSystemCredentials(c.getId(), new URL("http://example.org")));
    assertNull(CredentialsHelper
        .lookupSystemCredentials(c.getId(), new URL("http://nonexistent.url")));
  }

  @Test
  public void migrateCredentials()
      throws MalformedURLException {
    assertThat(CredentialsProvider.lookupStores(r.jenkins).iterator().next()
            .getCredentials(Domain.global()),
        empty());

    StandardUsernamePasswordCredentials c =
        CredentialsHelper
            .migrateCredentials("username", "password", new URL("http://example.org"));
