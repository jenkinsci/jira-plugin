...
public class DescriptorImplTest {
  ...
  public void doFillCredentialsIdItems() throws IOException {
    ...
    Domain domain = new Domain("example", "test domain",
        Arrays.asList(new HostnameSpecification("example.org", null)));
    ...
    CredentialsStore credentialsStore = CredentialsProvider.lookupStores(r.jenkins).iterator()
        .next();
    ...
    JiraSite.DescriptorImpl descriptor = r.jenkins
        .getDescriptorByType(JiraSite.DescriptorImpl.class);

    try (ACLContext ignored = ACL.as(User.get("admin"))) {
      ListBoxModel options = descriptor
          .doFillCredentialsIdItems(null, null, "http://example.org");
      assertThat(options.toString(), options, hasSize(3));
      assertTrue(options.toString(),
          listBoxModelContainsName(options, CredentialsNameProvider.name(c1)));
      ...
      assertTrue(options.toString(),
          listBoxModelContainsName(options, CredentialsNameProvider.name(c2)));
    }

    try (ACLContext ignored = ACL.as(User.get("alice"))) {
      ListBoxModel options = descriptor
          .doFillCredentialsIdItems(dummy, null, "http://example.org");
      assertThat(options.toString(), options, hasSize(1));
    }
