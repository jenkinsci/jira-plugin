...
public class UpdaterTest {
  ...
  public void getScmCommentsFromPreviousBuilds() {
    ...
    final MockEntry entry1 = new MockEntry("FOOBAR-1: The first build");
    {
      ...
      doReturn(new JiraCarryOverAction(
          new HashSet(Arrays.asList(new JiraIssue("FOOBAR-1", null)))))
    ...
    doAnswer((Answer<Object>) invocation -> {
      Comment rc = Comment.createWithGroupLevel((String) invocation.getArguments()[1],
    ...
    final Set<JiraIssue> ids = new HashSet(
        Arrays.asList(new JiraIssue("FOOBAR-1", null), new JiraIssue("FOOBAR-2", null)));
  ...
  /**
   * Tests that the generated comment matches the expectations - especially that the Jira id is not
   * stripped from the comment.
   */
  @Test
  ...
  public void comment() {
    ...
    doAnswer(answer).when(session)
        .addComment(Mockito.anyString(), Mockito.anyString(), Mockito.anyString(),
            Mockito.anyString());
    ...
    Set<? extends Entry> entries = new HashSet(
        Arrays.asList(new MockEntry("Fixed FOOBAR-4711")));
  ...
  public void issueIsRemovedFromCarryOverListAfterSubmission() throws RestClientException {
    ...
    Set<JiraIssue> issues = new HashSet(
        Arrays.asList(firstIssue, secondIssue, forbiddenIssue, thirdIssue));
    ...
    Answer answer = (Answer<Object>) invocation -> {
      Comment c = Comment.createWithGroupLevel((String) invocation.getArguments()[0],
    ...
    doAnswer(answer).when(session)
        .addComment(eq(firstIssue.getKey()), Mockito.anyString(), Mockito.anyString(),
            Mockito.anyString());
    doAnswer(answer).when(session)
        .addComment(eq(secondIssue.getKey()), Mockito.anyString(), Mockito.anyString(),
            Mockito.anyString());
    doAnswer(answer).when(session)
        .addComment(eq(thirdIssue.getKey()), Mockito.anyString(), Mockito.anyString(),
            Mockito.anyString());

    // issue for the caught exception
    doThrow(new RestClientException(new Throwable(), 404)).when(session)
        .addComment(eq(deletedIssue.getKey()), Mockito.anyString(), Mockito.anyString(),
            Mockito.anyString());
    doThrow(new RestClientException(new Throwable(), 403)).when(session)
        .addComment(eq(forbiddenIssue.getKey()), Mockito.anyString(), Mockito.anyString(),
            Mockito.anyString());
    ...
    updaterCurrent.submitComments(
        build, System.out, "http://jenkins", issues, session, false, false, groupVisibility,
        roleVisibility
    );
  ...
  /**
   * Test that workflow job - run instance of type WorkflowJob - can return changeSets using java
   * reflection api
   */
  @Test
  @WithoutJenkins
  public void getChangesUsingReflectionForWorkflowJob() throws IOException {
    ...
    List<ChangeLogSet<? extends Entry>> changesUsingReflection = RunScmChangeExtractor
        .getChangesUsingReflection(workflowRun);
  ...
  /**
   * Test formatting of scm entry change description when no format is provided (e.g. when null).
   */
  @Test
  @WithoutJenkins
  ...
  /**
   * Test formatting of scm entry change description when no format is provided (e.g. when empty
   * string).
   */
  @Test
  @WithoutJenkins
  ...
  /**
   * Test formatting of scm entry change description coverage primary wiki style
   * appendRevisionToDescription and appendAffectedFilesToDescription
   */
  @Test
