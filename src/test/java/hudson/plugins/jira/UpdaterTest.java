...
//        when(session.getGroup(Mockito.anyString())).thenReturn(new Group("Software Development", null));
        ...
        doAnswer(answer).when(session).addComment(eq(firstIssue.getId()), Mockito.anyString(), Mockito.anyString(), Mockito.anyString());
        doAnswer(answer).when(session).addComment(eq(secondIssue.getId()), Mockito.anyString(), Mockito.anyString(), Mockito.anyString());
        doAnswer(answer).when(session).addComment(eq(thirdIssue.getId()), Mockito.anyString(), Mockito.anyString(), Mockito.anyString());

        // issue for the caught exception
        doThrow(new RestClientException(new Throwable(), 404)).when(session).addComment(eq(deletedIssue.getId()), Mockito.anyString(), Mockito.anyString(), Mockito.anyString());
        doThrow(new RestClientException(new Throwable(), 403)).when(session).addComment(eq(forbiddenIssue.getId()), Mockito.anyString(), Mockito.anyString(), Mockito.anyString());
