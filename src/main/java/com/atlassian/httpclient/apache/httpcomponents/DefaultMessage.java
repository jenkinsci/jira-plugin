...
/**
 * An abstract base class for HTTP messages (i.e. Request and Response) with support for header and
 * entity management.
 */
abstract class DefaultMessage implements Message {
  ...
  public DefaultMessage(final Headers headers, final InputStream entityStream,
      Option<Long> maxEntitySize) {
  ...
  public String getEntity() throws IllegalStateException, IllegalArgumentException {
    ...
    if (hasEntity()) {
      ...
      final Charset charset =
          charsetAsString != null ? Charset.forName(charsetAsString) : Charset.forName(
              "UTF-8");
      try {
  ...
  public Message validate() {
    if (hasEntity() && headers.getContentType() == null) {
      throw new IllegalStateException(
          "Property contentType must be set when entity is present");
    }
    return this;
  }
