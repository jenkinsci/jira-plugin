...
public final class ApacheAsyncHttpClient<C> implements HttpClient, DisposableBean {
  ...
  public ApacheAsyncHttpClient(EventPublisher eventConsumer,
    ...
    this(eventConsumer, applicationProperties, threadLocalContextManager,
  ...
  public ApacheAsyncHttpClient(final Supplier<String> applicationName,
    ...
    try {
      ...
      throw new RuntimeException(
          "Reactor " + options.getThreadPrefix() + "not set up correctly", e);
    }
  }
  ...
  private HttpAsyncClientBuilder createClientBuilder() throws IOReactorException {
    ...
    final RequestConfig requestConfig = RequestConfig.custom()
        ...
        .setCookieSpec(
            options.getIgnoreCookies() ? CookieSpecs.IGNORE_COOKIES : CookieSpecs.DEFAULT)
        .setSocketTimeout((int) options.getSocketTimeout())
        .build();

    final HttpAsyncClientBuilder clientBuilder = HttpAsyncClients.custom()
        .setThreadFactory(ThreadFactories
            .namedThreadFactory(options.getThreadPrefix() + "-io", ThreadFactories.Type.DAEMON))
    ...
    if (Jenkins.getInstance() != null) {
      ...
      if (proxyConfiguration != null) {
        final HttpHost proxy = new HttpHost(proxyConfiguration.name,
        ...
        clientBuilder.setRoutePlanner(
            new JenkinsProxyRoutePlanner(proxy,
  ...
  private Registry<SchemeIOSessionStrategy> getFallbackRegistry(
  ...
  private ResponsePromise doExecute(final Request request) {
    ...
    return ResponsePromises
        .toResponsePromise(asyncClient.execute(op, new BasicHttpContext()).fold(
            throwable -> {
            ...
            httpResponse -> {
              final long requestDuration = System.currentTimeMillis() - start;
              publishEvent(request, requestDuration,
  ...
  private PromiseHttpAsyncClient getPromiseHttpAsyncClient(Request request) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Reactor " + httpClientOptions.getThreadPrefix() + "not set up correctly", e);
    }

    return new SettableFuturePromiseHttpPromiseAsyncClient<C>(nonCachingHttpClient,
  ...
  private static final class NoOpThreadLocalContextManager<C> implements
