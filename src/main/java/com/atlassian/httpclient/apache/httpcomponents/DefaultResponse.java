...
public final class DefaultResponse extends DefaultMessage implements Response {
  ...
  public DefaultResponse(Headers headers, InputStream entityStream, Option<Long> maxEntitySize,
    ...
    if (lengthString != null) {
      try {
        ...
        return parsedLength.flatMap(aLong -> {
          if (aLong < 0) {
            log.warn("Unable to parse content length. Received out of range value {}",
                aLong);
            return Option.none();
          } else {
