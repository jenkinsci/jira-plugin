...
public class JiraIssueMigrator extends Notifier {
    ...
    public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener) {
        ...
        try {
            ...
            listener.getLogger()
                    .printf("[Jira] Adding fixVersion %s for issues found from JQL `%s`", realRelease, realQuery);
            ...
            e.printStackTrace(listener.fatalError(
                    "[Jira] Unable to perform Jira operations (%s/%s): %s", realRelease, realProjectKey, e));
