...
class Updater {
  ...
  private static Set<JiraIssue> getJiraIssues(Set<String> ids, JiraSession session,
  ...
  boolean perform(Run<?, ?> build, TaskListener listener, AbstractIssueSelector selector) {
    ...
    try {
      ...
      LOGGER
          .log(Level.WARNING, "Error updating Jira issues. Saving issues for next build.", e);
  ...
  /**
   * Submits comments for the given issues. Removes from <code>issues</code> issues which have been
   * successfully updated or are invalid
   */
  void submitComments(
      Run<?, ?> build, PrintStream logger, String jenkinsRootUrl,
      Set<JiraIssue> issues, JiraSession session,
      boolean useWikiStyleComments, boolean recordScmChanges, String groupVisibility,
      String roleVisibility) throws RestClientException {
    ...
    for (JiraIssue issue : copy) {
      ...
      try {
        session.addComment(
            issue.getKey(),
            createComment(build, useWikiStyleComments, jenkinsRootUrl, recordScmChanges,
                issue),
        ...
        if (e.getStatusCode().or(0).equals(404)) {
          logger.println(issue.getKey()
              + " - Jira issue not found. Dropping comment from update queue.");
        }
  ...
  /**
   * Creates a comment to be used in Jira for the build. For example:
   * <pre>
   ...
   * </pre>
   */
  private String createComment(Run<?, ?> build, boolean wikiStyle, String jenkinsRootUrl,
      boolean recordScmChanges, JiraIssue jiraIssue) {
  ...
  private String getScmComments(boolean wikiStyle, Run<?, ?> run, boolean recordScmChanges,
      JiraIssue jiraIssue) {
    StringBuilder comment = new StringBuilder();
    for (ChangeLogSet<? extends Entry> set : RunScmChangeExtractor.getChanges(run)) {
      for (Entry change : set) {
        if (jiraIssue != null && !StringUtils
            .containsIgnoreCase(change.getMsg(), jiraIssue.getKey())) {
          continue;
        }
        comment.append(
            createScmChangeEntryDescription(run, change, wikiStyle, recordScmChanges));
      }
    }
    ...
    try {
      for (AffectedFile affectedFile : change.getAffectedFiles()) {
        ...
        if (affectedFile.getEditType() != null) {
          description.append("(").append(affectedFile.getEditType().getName())
              .append(") ");
        }
      ...
      LOGGER.warning(
          "Unsupported SCM operation 'getAffectedFiles'. Fall back to getAffectedPaths.");
  ...
  protected void appendChangeTimestampToDescription(StringBuilder description, JiraSite site,
      long timestamp) {
    DateFormat df = null;
