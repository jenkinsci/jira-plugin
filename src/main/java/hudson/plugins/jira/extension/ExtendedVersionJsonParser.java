...
public class ExtendedVersionJsonParser implements JsonObjectParser<ExtendedVersion> {
  ...
  public ExtendedVersion parse(JSONObject json) throws JSONException {
    ...
    return new ExtendedVersion(self, id, name, description, isArchived, isReleased, startDate,
        releaseDate);
  }

  private DateTime parseDate(String dateStr) {
    if (dateStr != null) {
      return dateStr.length() > "YYYY-MM-RR".length() ? JsonParseUtil.parseDateTime(dateStr)
