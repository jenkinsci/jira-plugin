...
public class JiraIssueUpdateBuilder extends Builder implements SimpleBuildStep {
  ...
  public void perform(Run<?, ?> run, FilePath workspace, Launcher launcher, TaskListener listener)
    ...
    String realWorkflowActionName = Util
        .fixEmptyAndTrim(run.getEnvironment(listener).expand(workflowActionName));
    ...
    if (StringUtils.isNotEmpty(realWorkflowActionName)) {
      listener.getLogger().println(
          Messages.JiraIssueUpdateBuilder_UpdatingWithAction(realWorkflowActionName));
    }

    listener.getLogger().println("[Jira] JQL: " + realJql);

    try {
      if (!site.progressMatchingIssues(realJql, realWorkflowActionName, realComment,
          listener.getLogger())) {
