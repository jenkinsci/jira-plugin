...
public class RunScmChangeExtractor {
  ...
  private static final String CANNOT_ACCESS_GET_CHANGESET_METHOD =
      "cannot call " + GET_CHANGESET_METHOD;
  ...
  static List<ChangeLogSet<? extends Entry>> getChangesUsingReflection(Run<?, ?> run) {
    ...
    for (Method method : run.getClass().getMethods()) {
      if (method.getName().equals(GET_CHANGESET_METHOD) && List.class
