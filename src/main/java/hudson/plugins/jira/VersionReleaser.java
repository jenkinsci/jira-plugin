...
public class VersionReleaser {
  ...
  protected boolean perform(Job<?, ?> project, String jiraProjectKey, String jiraRelease,
      String jiraDescription, Run<?, ?> run, TaskListener listener) {
    ...
    try {
      ...
      Optional<ExtendedVersion> sameNamedVersion = site.getVersions(realProjectKey).stream()
          .filter(
              version -> version.getName().equals(finalRealRelease) && version.isReleased())
          .findFirst();

      if (sameNamedVersion.isPresent()) {
        listener.getLogger()
            .println(Messages.VersionReleaser_AlreadyReleased(realRelease, realProjectKey));
      } else {
        listener.getLogger()
            .println(Messages.VersionReleaser_MarkingReleased(realRelease, realProjectKey));
   ...
   */
  protected void releaseVersion(String projectKey, String versionName, String versionDescription,
      JiraSession session) {
    if (session == null) {
    ...
    if (matchingVersion.isPresent()) {
      ...
      ExtendedVersion releaseVersion = new ExtendedVersion(version.getSelf(), version.getId(),
          version.getName(),
          !StringUtils.isEmpty(versionDescription) ? versionDescription
