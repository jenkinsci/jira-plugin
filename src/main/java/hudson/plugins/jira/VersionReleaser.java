    ...
    protected boolean perform(Job<?, ?> project, String jiraProjectKey, String jiraRelease, String jiraDescription, Run<?, ?> run, TaskListener listener) {
        ...
        try {
            ...
            if (sameNamedVersion.isPresent()) {
                ...
                final JiraProjectProperty jiraProjectProperty = project.getProperty(JiraProjectProperty.class);
                releaseVersion(realProjectKey, realRelease, realDescription, jiraProjectProperty.getJiraProjectSession());
            }
        } catch (Exception e) {
            listener.fatalError(
