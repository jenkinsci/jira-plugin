...
| # | Decision | Status |
|---|---|---|
| [0001](adr/0001-sonarcloud-analysis-for-fork-prs.md) | SonarCloud analysis for pull requests from forks — a two-workflow split so fork PRs get analysis without `SONAR_TOKEN` reaching fork-controlled code | accepted |
| [0002](adr/0002-jira-cloud-detection-and-search-api-routing.md) | Jira Cloud detection and search API routing — pass an explicit `isCloudVersion` to JRJC, widen the heuristic to `api.atlassian.com`, add a site-level override | proposed |
| [0003](adr/0003-future-of-the-vendored-atlassian-http-client.md) | Future of the vendored Atlassian HTTP client — keep and fix the fork in 3.x (it carries Jenkins proxy support upstream lacks), target HttpClient 5 in 4.0 | proposed |
...
| [0009](adr/0009-live-jira-cloud-e2e-as-release-gate.md) | Live Jira Cloud e2e suite as a release gate - a blocking step in the on-demand `cd.yaml` workflow, not on PRs (no secrets for forks) or on a schedule (litters the sandbox, nobody watching) | accepted |
...
[0006](adr/0006-http-client-lifecycle-and-jenkins-60536.md) is the archetype: the code it describes reads as a
